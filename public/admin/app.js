...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function updateGreetingHeading(userName) {
    ...
    const name = userName || localUser.name || 'Admin User';

    const greetingHeading = document.getElementById('admin-greeting-heading') || document.querySelector('h1.font-headline-lg-mobile');
    if (greetingHeading) {
  ...
  const localUser = JSON.parse(sessionStorage.getItem('civis_user') || '{}');
  const activeName = localUser.name || 'Admin User';
  ...
  submitBtns.forEach(btn => {
    ...
    newBtn.addEventListener('click', () => {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (pos) => {
            openReportModalAtCoords(pos.coords.latitude, pos.coords.longitude, analysis.title, dbCategory, customLocation, analysis.description, 'gps', pos.coords.accuracy || null);
          },
          (err) => {
            console.warn("GPS Geolocation failed/denied:", err?.message);
            openReportModalAtCoords(null, null, analysis.title, dbCategory, customLocation, analysis.description, null, null);
          },
          GEOLOCATION_OPTIONS
        );
      } else {
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md relative animate-in fade-in zoom-in-95 duration-200">
      ...
      <form id="edit-profile-form" class="flex flex-col gap-4">
        <div>
          ...
          <input required id="edit-form-name" value="${localUser.name || ''}" class="w-full p-3 border border-border-subtle rounded-xl outline-none focus:ring-2 focus:ring-primary/40" placeholder="e.g. Admin Name">
        </div>
        <div>
...
function openReportModal() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (pos) => openReportModalAtCoords(pos.coords.latitude, pos.coords.longitude, '', '', '', '', 'gps', pos.coords.accuracy || null),
      (err) => {
        console.warn("GPS Geolocation failed/denied:", err?.message);
        openReportModalAtCoords(null, null, '', '', '', '', null, null);
      },
      GEOLOCATION_OPTIONS
    );
  } else {
...
function openReportModalWithDetails(title, category) {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (pos) => openReportModalAtCoords(pos.coords.latitude, pos.coords.longitude, title, category, '', '', 'gps', pos.coords.accuracy || null),
      (err) => {
        console.warn("GPS Geolocation failed/denied:", err?.message);
        openReportModalAtCoords(null, null, title, category, '', '', null, null);
      },
      GEOLOCATION_OPTIONS
    );
  } else {
...
function openReportModalAtCoords(lat, lng, defaultTitle = '', defaultCategory = '', defaultLocation = '', defaultDesc = '', locationSource = null, locationAccuracy = null) {
    ...
    if (isHidden) {
      ...
      if (typeof L !== 'undefined') {
        ...
        if (pickerDiv) {
          ...
          const targetLng = isValidCoordinate(currentLat, currentLng) ? targetLng : 73.8567;
